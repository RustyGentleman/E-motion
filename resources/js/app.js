...
function resize() {
	...
	canvas.width = document.body.scrollWidth/UPSCALE
	canvas.height = document.body.scrollHeight/UPSCALE
...
function go(delta) {
	...
	for (let y = top; y < height + top; ++y) {
		for (let x = 0; x < width; ++x) {
			...
			let r = Math.floor(noiseR * 128) & 0xff
			pixBuf[i++] = r | 0xff000000
		}
	}
	ctx.putImageData(pixels, 0, 0)
	// requestAnimationFrame(go)
	lastTime = performance.now()
	timeout = setTimeout(go, 64)
}
go();

//# Original code

//* Click, enter and space on label to input
...
for (const spin of document.querySelectorAll('.spin')) {
	let f = 0
	const bars = Array.from(spin.querySelectorAll('.bar'))
	let speeds = [1/9, 1/7, 1/5, 1/3]
	speeds = [...speeds, ...speeds.reverse().slice(1)]

	requestAnimationFrame(incrementSpin)
	function incrementSpin() {
		for (const [i, bar] of bars.entries()) {
			let w = f * speeds[i % 6] * MULT
			bar.style.setProperty('--w', (w % 100) + '%')
		}
		f++
		setTimeout(incrementSpin, UPDATE)
	}
}